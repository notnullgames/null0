...
| grain          | `carts/grain/simple`         | `main.gr`    | `grain`          | `provide let load = () => {}`           | low-level: colors are memory addresses                 |
...
- **Colors**: high-level languages give you a `Color` value. In WAT/walt/grain a
  Color is a _pointer to 4 bytes_ you must write into memory yourself - read
  those examples closely.
