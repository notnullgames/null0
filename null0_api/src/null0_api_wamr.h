...
static void wamr_null0_load_sfx(wasm_exec_env_t exec_env, SfxParams* outSfxParamsValue, char* filename) {
  params_from_sfx_to_wasm(null0_load_sfx(filename), outSfxParamsValue);
}
...
static uint32_t wamr_null0_file_read(wasm_exec_env_t exec_env, char* filename, uint32_t* bytesRead) {
  ...
  if (bytesRead > 0) {
    uint32_t out = wasm_runtime_module_dup_data(module_inst, bytes, *bytesRead);
    return out;
  }
  return 0;
}
